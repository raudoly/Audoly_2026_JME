...
y = zeros(p.np+1,ks.T);
dL = [st.dL,zeros(p.np,ks.T)];

% Pre-allocate some computations outside loop
stS_n = repmat(st.S,1,ks.ns);
stU_n = repmat(st.U,1,ks.ns);

h_n = zeros(p.np,ks.ns);
c_n = zeros(p.np,ks.ns);
...
for t = 1:ks.T
    ...
    [chi_t,~,h_t,~,q_t] = solve_firm_policy_shocks(phi_t,dL(:,t),ks.sim_delta(t),ks.sim_c0(t),p);

    % Update measure of workers given these policies
    dL(:,t+1) = p.tP*(chi_t.*((1 - q_t + h_t).*(1 - p.mu).*(1 - ks.sim_delta(t)).*dL(:,t) + p.mu*p.dP0));
    X(:,t+1) = build_endogenous_regressors(dL(:,t+1),ks,p);
...
for t = ks.T:-1:1
    ...
    Y_n = 1 - sum(dL(:,t+1)) + (1 - ks.delta).*p.s.*(p.sum_bl*(chi_n.*dL_n));
    V_n = (1 - ks.delta).*p.s.*(p.sum_bl*(chi_n.*phi_n.*dL_n))./Y_n;

    for k = 1:ks.ns
        h_n(:,k) = p.mc_inv(chi_n(:,k).*(phi_n(:,k) - V_n(:,k)),ks.c0(k),p.c1);
        c_n(:,k) = p.c(h_n(:,k),ks.c0(k),p.c1);
    end

    lambda_dF_n = (1 - ks.delta).*chi_n.*h_n.*dL_n./Y_n;
    q_n = p.s*(p.sum_ab*lambda_dF_n);

    % Continuation values
    U_cont = U_n + p.mu*sum(chi_n.*phi_n.*p.dP0,1) + (1 - p.mu)*sum(V_n.*lambda_dF_n,1);

    S_cont = -c_n + (1 - q_n).*phi_n + h_n.*(phi_n - V_n) + p.s*(p.sum_ab*(V_n.*lambda_dF_n));
    S_cont = U_n + p.mu*sum(chi_n.*phi_n.*p.dP0,1) + (1 - p.mu)*(1 - ks.delta).*chi_n.*S_cont;

    % Value functions, in log-deviations from stationary equilibrium
    U_t = ks.sim_b(t) + ks.sim_beta(t)*U_cont*ks.tQ(:,ks.sim_state(t));
    S_t = ks.sim_omega(t)*p.p + ks.sim_beta(t)*p.P*S_cont*ks.tQ(:,ks.sim_state(t));

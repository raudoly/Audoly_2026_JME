...
for t = 1:T
    ...
    % Solve for shocks
    problem.objective = @(x) gap_to_series(x, dL_t, log_omega_tm1, dat_ts(t), target_var, ks, st, p);
    ...
    [log_omega_t, dLp_t] = simulate_period(eps_omega_t, dL_t, log_omega_tm1, ks, st, p);

    switch target_var
        case 'lngdp_hpf_dev'
            sim_ts(t) = sim_gdp_dev(log_omega_t, dLp_t, st, p);
        case 'u_hpf_dev'
            sim_ts(t) = sim_u_dev(dLp_t, st);
        otherwise
            sim_ts(t) = sim_lp_dev(log_omega_t, dLp_t, st, p);
    end
...
% Check statistical properties of sequence of shocks fitting
...
function [log_omega_t, dLp_t] = simulate_period(eps_omega_t, dL_t, log_omega_tm1, ks, st, p)

m_t = build_endogenous_regressors(dL_t, ks, p);   
...
[chi_t, ~, h_t, ~, q_t] = solve_firm_policy_shocks(phi_t, dL_t, delta_t, c0_t, p);
dLp_t = chi_t.*((1 - q_t + h_t).*(1 - p.mu).*(1 - delta_t).*dL_t + p.mu*p.dP0);

function gap = gap_to_series(eps_omega_t, dL_t, log_omega_tm1, dat_dev, target_var, ks, st, p)
...
switch target_var
    case 'lngdp_hpf_dev'
        sim_dev = sim_gdp_dev(log_omega_t, dLp_t, st, p);
    case 'u_hpf_dev'
        sim_dev = sim_u_dev(dLp_t, st);
    otherwise
        sim_dev = sim_lp_dev(log_omega_t, dLp_t, st, p);
end
...
function y_dev = sim_gdp_dev(log_omega_t, dLp_t, st, p)

y_t = log_omega_t + log(sum(p.p.*dLp_t));
y_st = log(sum(p.p.*st.dLp));

y_dev = y_t - y_st; 


function y_dev = sim_lp_dev(log_omega_t, dLp_t, st, p)

y_t = log_omega_t + sum(p.lnp.*dLp_t)/sum(dLp_t);
y_st = sum(p.lnp.*st.dLp)/sum(st.dLp);

y_dev = y_t - y_st; 

function y_dev = sim_u_dev(dLp_t, st)
